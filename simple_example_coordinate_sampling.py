...
density_funcs = {
    ...
    r"manifold actual density": lambda a: np.exp(-1 / 2 * np.linalg.norm(a, axis=1) ** 2)
    * manifold_density_reweight_desired(a),
    ...
    r"manifold proj theta_12": lambda a: np.exp(-1 / 2 * a[:, 1] ** 2)
    * (manifold_density_reweight_actual_x_param(a) + manifold_density_reweight_actual_y_param(a)),
}
